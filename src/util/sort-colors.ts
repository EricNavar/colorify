...
// Sorting
function blendRgbaWithWhite(rgba: string) {
...
const reduceClusters = (clusters: Cluster[]) => {
  return clusters.reduce((acc: string[], curr: Cluster) => {
    const colors = curr.colors.map((color: Color) => color.hex);
    return [...acc, ...colors];
  }, []);
};

function sortWithClusters(colorsToSort: string[]) {
  ...
  return reduceClusters(clusters);
}

export const sortedColors = sortWithClusters(colors);
